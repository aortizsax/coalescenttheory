...
FINAL 2 increase and decrease population
@author: aorti
...
def coalesce(N,nodes,i,genN,gens,coalT):
    n = len(nodes)  # n0 - i +n0 ##i starts at n0 in this code
    Ncurr = genN[int(-coalT)]#population at time 
    Twaitn = Ncurr / nchoose2(n)  # can simplify if write out n choose 2
    Twaitn = round(Twaitn,2)
    print("Twaitn1:",Twaitn)
    
    if -coalT-Twaitn < -1 * len(genN):
        Ntemp = genN[0]#population at time 
        Twaitn2 = Ntemp / nchoose2(n)  # can simplify if write out n choose 2
        Twaitn2 = round(Twaitn2,2)
        
        Twait = (Twaitn + Twaitn2) / 2
        Navg = avg(genN[0:int(-coalT)])
    else:
        Ntemp = genN[int(-coalT-Twaitn)]#population at time 
        Twaitn2 = Ntemp / nchoose2(n)  # can simplify if write out n choose 2
        Twaitn2 = round(Twaitn2,2)
        
        Twait = (Twaitn + Twaitn2) / 2
        Navg = avg(genN[int(-coalT-Twaitn):int(-coalT)])

    
    coalT += Twaitn
    print('generations',coalT)
...
if __name__ == '__main__':
    ...
    n0 = 13#sample size
    gens = 0
    coalT = 1 # simulaion time upkeep
    
    #partial sum of 
    genscheck = Nmax * (2 - 2 / (n0))
    
    genN = [0] * int(genscheck)
    for j in range(int(genscheck)):
        genN[j] = int(0.541125*j+Nmin)
    print(genN[0],genN[-1])
    
    genN = [0] * int(genscheck)
    for j in range(int(genscheck)):
        genN[j] = int(-0.541125*j+Nmax)
    print(genN[0],genN[-1])
    ...
    while len(samplednodes)>1:
        samplednodes, i, coalT = coalesce(N,samplednodes,i,genN,gens,coalT)
