...
if __name__ == '__main__': 
    ...
    last_edge = pop_tree_str.split(':')[-1][:-1] 
    root = Population('D',last_edge)
    pop_tree = Tree(root) 
    
    working_str = pop_tree_str.split(':'+last_edge+';')[0]
    
    c = 1
    for subtree in working_str[1:].split(','): 
        print(subtree)
        print('child of:',subtree.count('(')) 
        print('child of:',subtree.count(')')) 
        if subtree.count('('):
            tmp_node = Population(intLab[c])
            tmp_node.parent = pop_tree.head
            pop_tree.head.child1 = tmp_node
            c += 1 
        elif subtree.count(')'):
            edge_length = subtree[3]
            tmp_node.child2 = Node(subtree[0],edge_length)
    #print(dir(pop_tree.inorder(pop_tree.head)))
    print(pop_tree) 
    print(tmp_node.name)
    print(tmp_node.child2)
    print(pop_tree)
    print(pop_tree)
