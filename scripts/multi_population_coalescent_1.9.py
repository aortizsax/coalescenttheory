...
def multiple_population_coalescence(N,nodes,i,max_time,coal_time):
    '''
    coalescent method for random set of nodes 
    '''
...
def read_population_tree(filename):
    return
...
if __name__ == '__main__': 
    ...
    pop_tree_str = '((A:50,B:50):50,C:100):50;' 
    ...
    c = 0
    num_merge = 0
    print(find_parents(pop_tree_str))
    for i,j in find_parents(pop_tree_str).items():
        tmp_name = intLab[c]
        j = find_parents(pop_tree_str)[i]
        print(i,j)
    ...
    while len(samplednodes) != 1:
        ...
        for sampled_population in samplednodes:
            ...
            while (len(sampled_population)>1)&(not trunc_T):
                ...
                sampled_population, ii, coal_time, trunc_T = multiple_population_coalescence(N,
                                                              sampled_population,
